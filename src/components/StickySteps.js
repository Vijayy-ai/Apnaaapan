...
const StickySteps = ({ steps = [], onActiveChange, onProgress }) => {
  ...
  const sectionHeight = `calc(100vh * ${Math.max(1, steps.length - 0.5)})`;

  return (
    <div ref={sectionRef} className="relative" style={{ height: sectionHeight }}>
      <div className="sticky top-36 md:top-40 lg:top-44 max-h-[64vh] mt-16 md:mt-20 lg:mt-24">
        <div className="relative bg-white rounded-3xl shadow-sm border border-[#E6E1D6] overflow-hidden h-full">
          {steps.map((step, i) => (
            <div
              key={i}
            ...
            >
              <div className="flex flex-col md:flex-row items-center gap-6 md:gap-10 h-full">
                <div className="md:w-1/2 order-2 md:order-1">
                  ...
                  <h4 className="font-serif text-[#0D1B2A] text-3xl md:text-4xl font-bold mb-4">{step.title}</h4>
                  <p className="text-[#22223b] leading-7 md:leading-8">{step.description}</p>
                </div>
                <div className="md:w-1/2 order-1 md:order-2 flex items-center justify-center">
                  <img src={step.image} alt={`${step.title} step`} className="w-full h-auto max-h-56 md:max-h-72 object-contain" />
                </div>
              </div>
            </div>
          ))}
